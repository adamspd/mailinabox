...
if [ ! -f "$db_path" ]; then
	...
	echo "CREATE TABLE webauthn_credentials (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, credential_id TEXT NOT NULL, public_key TEXT NOT NULL, sign_count INTEGER DEFAULT 0, transports TEXT, label TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);" | sqlite3 "$db_path";
	echo "CREATE TABLE webauthn_challenges (key TEXT PRIMARY KEY, options BLOB NOT NULL, expires_at DATETIME NOT NULL);" | sqlite3 "$db_path";
